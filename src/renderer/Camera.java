   ...
   //	private int numOfRays;
	...
	//camera:
		...
		public Camera(Point _p0, Vector _vTo, Vector _vUp )
		{
			...
			vTo = _vTo.normelaize();	//camera's vectors must be normalized
			vUp = _vUp.normelaize();	//camera's vectors must be normalized
			vRight = vTo.crossProduct(vUp).normelaize();//we used crossProduct, since vRight is the normal to vTo and vUp plane.
		}
